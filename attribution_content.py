"""Gender Attribution
    ...
    Usage:
    attribution_content.py --words
    attribution_content.py --chars=<n>
    attribution_content.py (-h | --help)
    attribution_content.py --version
...
RESULTS_CONTENT_DIR = "results_content"
os.makedirs(RESULTS_CONTENT_DIR, exist_ok=True)

def plot_and_save_metric(all_metrics, metric_name, classes, RESULTS_CONTENT_DIR):
    import matplotlib.pyplot as plt
    ...
    filename = os.path.join(RESULTS_CONTENT_DIR, f"{metric_name.lower().replace(' ', '_')}_over_runs.png")
...
def evaluate_model(test_documents, feature_type, ngram_size, classes, priors, conditional_probabilities):
    ...
    if feature_type == "words":
        filename = f"confusion_matrix_content_{feature_type}.png"
    elif feature_type == "chars":
        filename = f"confusion_matrix_{feature_type}_{ngram_size}.png"

    filepath = os.path.join(RESULTS_CONTENT_DIR, filename)
...
if __name__ == '__main__':
    ...
    datapath = "Data/preprocessed/content_wd/Gender_based_texts"
    ...
    if feature_type == "chars":
        csv_filename = f"evaluation_summary_content_chars_{ngram_size}.csv"
    else:
        csv_filename = "evaluation_summary_content_words.csv"

    csv_filepath = os.path.join(RESULTS_CONTENT_DIR, csv_filename)
