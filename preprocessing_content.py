...
def preprocess_text(text):
    ...
    return " ".join(
        token.lemma_.lower()
        for token in doc
        if token.pos_ in {'NOUN', 'VERB', 'ADJ', 'ADV'} and not token.is_stop and token.is_alpha
    )
...
contentwd_folder = os.path.join(base_directory, "preprocessed", "content_wd")
os.makedirs(contentwd_folder, exist_ok=True)
...
def char_name_extractor(contentwd_folder):
    ...
    if characters :
        ...
        characters_name_file = os.path.join(name_dir, "characters_content.txt")
